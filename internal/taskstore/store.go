...
func (s *Store) migrate() error {
	...
	// R6: Dedup duplicate run_id before creating UNIQUE INDEX.
	s.db.Exec("DELETE FROM run_records WHERE rowid NOT IN (SELECT MIN(rowid) FROM run_records GROUP BY run_id)")
